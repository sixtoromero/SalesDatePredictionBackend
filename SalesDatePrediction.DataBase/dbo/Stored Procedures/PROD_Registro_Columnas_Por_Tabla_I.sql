...
BEGIN
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN c.CHARACTER_MAXIMUM_LENGTH IS NOT NULL THEN '(' + CAST(c.CHARACTER_MAXIMUM_LENGTH AS NVARCHAR(10)) + ')' ELSE '' END AS Size,
			c.IS_NULLABLE AS IsNullable,
			CASE WHEN pk.COLUMN_NAME IS NOT NULL THEN 'Primary Key' ELSE '' END AS IsPrimaryKey,
			CASE WHEN fk.COLUMN_NAME IS NOT NULL THEN 'Foreign Key' ELSE '' END AS IsForeignKey,
